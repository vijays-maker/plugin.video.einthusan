...
def get_movies_and_music_videos_helper(name, url, language, mode, page):
    ...
    # Bit of a hack
    MOVIES_URL = "https://www.einthusan.ca/movies/watch/"
...
def add_movies_to_list(movie_ids, bluray):
    ...
    BASE_URL = 'http://www.einthusan.ca/movies/watch/'
    if (bluray):
        UHD_STR = '/?uhd=true'
    else:
        UHD_STR = ''
    for m_id in movie_ids:
        ...
        addDir(name, BASE_URL + str(m_id) + UHD_STR ,2, COVER_BASE_URL + image)
...
def show_search_box(name, url, language, mode):
    ...
    if (keyb.isConfirmed()):
        ...
        myheaders={'Origin':'https://einthusan.ca','Referer':'https://einthusan.ca/movie/browse/?'+url,'User-Agent':'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36'}
...
def play_video(name, url, language, mode):
    ...
    if isithd=='itshd':
        ...
        if ret ==0:
            # isithd = 'itshd'
            myheaders={'Origin':'https://einthusan.ca','Referer':'https://einthusan.ca/movie/browse/?lang=hindi','User-Agent':'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36'}
...
def get_movie(s, mainurl, mainurlajax, myheaders=None):
    import HTMLParser
    import json
    #htm=getUrl(mainurl,headers=headers,cookieJar=cookieJar)
    #print(mainurl) 
    #print(mainurlajax) 
    htm = s.get(mainurl, headers=None, cookies=s.cookies).text.encode('utf-8')
    #xbmc.log("VIJAY+++++++++******************===============", level=xbmc.LOGNOTICE)
    #xbmc.log(mainurl, level=xbmc.LOGNOTICE)
    #xbmc.log(mainurlajax, level=xbmc.LOGNOTICE)
    ...
    postdata={'xEvent':'UIVideoPlayer.PingOutcome','xJson':jdata,'arcVersion':'10','appVersion':'236','gorilla.csrf.Token':gid}
	...
	if location != 'No Preference':
		if location == 'Dallas':
			...
			servers = [28,34,43]
                ...
                #xbmc.log("VIJAY_START_1+++++++++******************===============", level=xbmc.LOGNOTICE)
                #xbmc.log(lnk, level=xbmc.LOGNOTICE)
                server_n = lnk.split('.einthusan.io')[0].strip('https://cdn')
                #xbmc.log(server_n, level=xbmc.LOGNOTICE)
                #xbmc.log("VIJAY_END_1+++++++++******************===============", level=xbmc.LOGNOTICE)
		SERVER_OFFSET = []
		if int(server_n) > 100:
...
def login_info(s, referurl):
    ...
    myheaders={'Host':'einthusan.ca', 'Origin':'https://einthusan.ca','Referer':referurl,'User-Agent':'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36'}
    
    htm = s.get('https://einthusan.ca/login/?lang=tamil', headers=myheaders, allow_redirects=False).content
    csrf=re.findall('data-pageid=["\'](.*?)["\']',htm)[0]
    if '&#43;' in csrf: csrf = csrf.replace('&#43;', '+')
    
    body = {'xEvent':'Login','xJson':'{"Email":"'+username+'","Password":"'+password+'"}', 'arcVersion':10, 'appVersion':236,'tabID':csrf+'48','gorilla.csrf.Token':csrf}
    myheaders['X-Requested-With']='XMLHttpRequest'
    
    dialog = xbmcgui.Dialog()
    ret = dialog.select('here-1', ['body','Play HD/SD'])
    
    myheaders['Referer']='https://einthusan.ca/login/?lang=tamil'
    html2= s.post('https://einthusan.ca/ajax/login/?lang=tamil',headers=myheaders,cookies=s.cookies, data=body,allow_redirects=False) 
    
    html3=s.get('https://einthusan.ca/account/?flashmessage=success%3A%3A%3AYou+are+now+logged+in.&lang=tamil', headers=myheaders, cookies=s.cookies)
    
    csrf3 = re.findall('data-pageid=["\'](.*?)["\']',html3.text)[0]
    body4 = {'xEvent':'notify','xJson':'{"Alert":"SUCCESS","Heading":"AWESOME!","Line1":"You+are+now+logged+in.","Buttons":[]}', 'arcVersion':10, 'appVersion':236,'tabID':csrf+'48','gorilla.csrf.Token':csrf3}
    html4 = s.post('https://einthusan.ca/ajax/account/?lang=tamil', headers=myheaders, cookies=s.cookies, data=body4)
